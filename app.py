...
def load_marine_model():
    ...
    if len(classes) != 484:
        st.error(
            f"marine_labels.txt must have exactly 484 lines to match the trained model. Found {len(classes)}."
        )
        st.stop()
    model = models.resnet18(weights=None)
    model.fc = torch.nn.Linear(model.fc.in_features, 484)
    model.load_state_dict(
        torch.load("models/marine_model_retrained_weights.pth", map_location=DEVICE)
    )
    model.eval()
...
def predict_image(image, model, classes, image_size, normalize=True):
    ...
    with torch.no_grad():
        ...
        predictions = [
            (classes[i], float(prob)) for prob, i in zip(top3_prob[0], top3_idx[0])
        ]
...
def get_gradcam(model, input_tensor, pred_idx, model_type):
    ...
    grayscale_cam = cam(
        input_tensor=input_tensor, targets=[ClassifierOutputTarget(pred_idx)]
    )[0]
...
if image_file:
    ...
    predictions, input_tensor = predict_image(
        image, model, classes, image_size, normalize=normalize
    )
